...
class ExtDiskMount(DiskMount):
    ...
    def __format_filesystem(self):
        ...
        if self.fstype.startswith('ext'):
            args = args + ['-F', '-L', self.fslabel[0:16], '-m', '1', '-b',
                           str(self.blocksize)]
        elif self.fstype == 'xfs':
            args = args + ['-L', self.fslabel[0:12], '-b', 'size=%s' %
                           str(self.blocksize)]
        elif self.fstype == 'btrfs':
            args = args + ['-L', self.fslabel[0:255]]
        elif self.fstype == 'f2fs':
            ver = rcall(['mkfs.f2fs', '-V'])[0].split()[1].replace('.', '')
            if int(ver) >= 1140:
                c = ',compression'
            else:
                c = ''
            args = args + ['-f', '-l', self.fslabel[0:512],
                           '-O', 'extra_attr' + c]
