...
shortusage() {
    ...
    livecd-iso-to-disk [--help] [--format [<size>[,fstype[,blksz]]] [--msdos]
                       [--efi] [--noesp] [--reset-mbr] [--multi]
...
usage() {
    ...
    The script may be run in simplest form with just the two arguments:
             ...
             <target partition/device>
                 ...
                 may also be used.  (Issue the lsblk -pf command to get a listing
    ...
    construct for block devices may be used.  Multiple devices may be targeted
    simultaneously through a Device-mapper mirror target.  The source image may
    be either a LiveOS .iso file, or another reference to a LiveOS image, such
    as the device node for an attached device installed with a LiveOS image,
    its mount point, a loop device backed by a file containing an installed
    LiveOS image, or even the currently-running LiveOS image.
    ...
    --format [sizemb[,fat|vfat|msdos|ext[432]|btrfs|xfs|f2fs[,blksz]]]
        Formats the target device, creates an MS-DOS partition table or GUID
        partition table (GPT), if the --efi option is passed, creates 1 to 3
        partitions, and invokes the --reset-mbr action.

        NOTE WELL: All current disk content will be lost.

          Partition 1 is fstype formatted (default ext4),
            labelled as before or requested, flagged as bootable, and sized as
            requested or as available, with optional block size.
...
cleansrc() {
    if [[ $RFSMNT == $IMGMNT ]]; then
        IMGMNT=$i
        umount $RFSMNT && rmdir $RFSMNT
    fi
    umount $IMGMNT && rmdir $IMGMNT
}
        ...
        *)
            if ! type objcopy >/dev/null 2>&1; then
                if [[ $fstype == f2fs ]]; then
                    ...
                    package installed to build the UEFI executable for use as
                    an EFI Boot Stub, which is currently required to boot a
                    f2fs-formatted image.\n
                    Press Ctrl C to abort, or Enter to continue...\n\n'
                    read
                else
                    nouefi=nouefi
...
mkfs_config() {
    ...
    case $fstype in
        ...
        xfs)
            ...
            if [[ -n $fn ]]; then
                falloc $sz "$fn"
                ops+=" -K -l internal -d file=0,name=$fn" #,size=$((sz/${bs:=4096}))b"
            fi
            ;;
        btrfs)
...
checkFilesystem() {
    ...
    if [[ -n ${format[1]} ]] && [[ -z $skipcopy ]]; then
        if [[ part == $t ]]; then
            ...
            device, '%s'.\n
            Please designate a whole device path as your target for formatting.
            Exiting...\n" $dev $device
            exitclean
        fi
    ...
    case $TGTFS in
        ...
        ext[432]|btrfs|xfs|f2fs)
            ...
            ;;&
        btrfs)
            # Compression of $TGTMNT/syslinux interferes with EXTLINUX booting,
            # but is ok for UEFI grub booting.
...
checkMounted() {
    ...
    if [[ $SRC -ef $live_mp && -z $livedev ]] ||
        ...
        tgtdev=$livedev
    elif [[ $TGTDEV -ef $srcdev && -n ${format[1]} ]]; then
        printf '\n    NOTICE:
...
if [[ $2 == live ]]; then
    TGTDEV=$(realpath $(readlink /run/initramfs/livedev))
else
    TGTDEV=$(readlink -f "$2") || :
fi
...
if [[ -n ${format[1]} && -z $skipcopy ]]; then
    ...
    if [[ -z $noesp ]]; then
        ...
        if [[ -f $l2 ]]; then
            ...
            case $TGTFS in
                f2fs)
                    # Allow space for a UEFI executable, which can be produced
                    # by dracut --uefi, for use as an EFI Boot Stub.
                    # 128 MiB per 16 GiB of device_size for multi boot stubs.
                    ((p2s+=((f/(1<<34))+1)*(1<<27)))
            esac
...
config_efi() {
    ...
    if [[ $TGTMNT/EFI -ef $SRCMNT/EFI ]]; then
        ...
        cp -Tru $SRCMNT$EFI_BOOT $TGTMNT$T_EFI_BOOT
...
if [[ -z $skipcopy ]]; then
    ...
    for f in $(find $SRCMNT/images); do
        if [[ -d $f ]]; then
            if ! [[ -d $TGTMNT/$multi${f#$SRCMNT} ]]; then
...
if [[ -n $BOOTCONFIG_EFI ]]; then
    ...
    f+="/^\s*insmod\s+(ext2|fat|xfs|f2fs|btrfs)\s*$/ s;(ext2|fat|xfs|f2fs|btrfs);$m;
       "
...
if [[ -n $BOOTCONFIG_EFI ]]; then
    if [[ -b $p2 ]]; then
        d=$(mktemp -d)
        mount $p2 $d
        if [[ $nouefi != nouefi ]] && [[ $TGTFS == f2fs ]]; then
            # Build a UEFI executable for use as an EFI Boot Stub.
            mount -r "$SRCIMG" $IMGMNT || exitclean
            if ! [[ $flat_squashfs ]]; then
                RFSMNT=$(mktemp -d /run/imgtmp.XXXXXX)
                for f in "$IMGMNT/$srcdir/rootfs.img" \
                         "$IMGMNT/$srcdir/ext3fs.img"; do
                    if [[ -s $f ]]; then
                        mount -r "$f" $RFSMNT || exitclean
                        i=$IMGMNT
                        IMGMNT=$RFSMNT
                        break
                    fi
            ...
            [[ $efi ]] && printf "\nNOTICE:  $efi\n\n" && cleansrc && exitclean
            if [[ -d /sys/firmware/efi ]] && ! [[ $force ]]; then
                p=$srcdev
                [[ $SRCFS == iso9660 ]] && p=$(findmnt -no SOURCE $SRCMNT)
                efi=$label-$LIVEOS-$(blkid -s LABEL -o value $p)
                f="efibootmgr --create --disk $device --part 2 \
--loader \\linux_$LIVEOS.efi --label "
                IFS=: read -r -p "
                ATTENTION:

                Flash-Friendly File System (F2FS) formatted devices fail
                to boot with 2020-21 versions of SYSLINUX-EXTLINUX or GNU GRUB.
                ...
                input a substitute for the proposed label:
    ...
    if [[ -b $p3 ]]; then
        ...
        for f in $TGTMNT$T_EFI_BOOT/BOOT.conf $TGTMNT$T_EFI_BOOT/grub.cfg; do
            [[ -f $f ]] && cp $f $d$T_EFI_BOOT
        done
        cp $TGTMNT$T_EFI_BOOT/grub.cfg $d/System/Library/CoreServices
        [[ -f $BOOTCONFIG_EFI.prev ]] && cp $BOOTCONFIG_EFI.prev \
            $d$T_EFI_BOOT
        umount $d && rmdir $d
        fsck.hfsplus -ydfp $p3
    fi
